...
- `biz` never imports `api/...`, `service`, or `data`. The repo interface
  declared here is the inversion seam.
...
- Owns typed errors built with `errors.NotFound` / `errors.BadRequest`
  with Go error reason constants.
...
- Construct gRPC servers, apply middleware, register services. No
...
4. **Wiring**: register the repo constructor in `data.ProviderSet`, the
   ...
   register gRPC services in `internal/server`.
...
`*.pb.go`, `*_grpc.pb.go`, or `wire_gen.go`.
...
- Error reasons: declared as Go constants in `biz`,
  surfaced as `Err<Resource><Cause>`.
